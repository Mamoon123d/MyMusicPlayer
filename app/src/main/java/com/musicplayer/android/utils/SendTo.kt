...
open class SendTo(private val context: Context) {
    ...
    open fun isAppAvailable(context: Context, appName: String?): Boolean {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                val app = pm.getInstallSourceInfo(appName!!).installingPackageName
                Log.d("TAG", "isAppAvailable: " + app)
            }

            @Suppress("DEPRECATION")
            pm.getInstallerPackageName(appName!!)
    ...
    open fun sendVideoFile(filePath:String){
